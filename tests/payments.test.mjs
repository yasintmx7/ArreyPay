...
import {parseAmount,formatAmount,equalShare,gasWeiToUSDC,requiredBalance} from '../web/amounts.js';
...
test('money parsing and gas budgeting stay exact',()=>{assert.equal(parseAmount('0.000001'),1n);assert.equal(parseAmount('20.25'),20250000n);for(const value of ['-1','0','1e3','1.0000001','NaN','1000001',''])assert.throws(()=>parseAmount(value));assert.equal(formatAmount(12345678n),'12.345678');assert.equal(equalShare(10n,3,0n,2n),2n);assert.equal(gasWeiToUSDC(1n),1n);assert.equal(gasWeiToUSDC(1000000000000n),1n);assert.equal(gasWeiToUSDC(1000000000001n),2n);assert.equal(requiredBalance(1000000n,1000000000000n),1000002n);});
