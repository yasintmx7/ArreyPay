...
- Circle Bridge Kit + viem adapter for Ethereum Sepolia / Base Sepolia to Arc Testnet through CCTP. Official faucet link; incomplete returned bridge results can be resumed in the same browser tab after refresh.
...
Node.js 22+, then `npm ci`, `npm run build`, `npm run dev`. Build output is `dist`; source is `web`. Contracts compile before each build using the locked Solidity compiler, optimizer 200, EVM Paris target.

`npm test` runs money parsing and multi-wallet escrow lifecycle tests on a local EVM with a USDC fixture at Arc's address. It covers overpayment, unauthorized cancellation, refund ownership, expiry, duplicate refunds and failed-settlement rollback. These tests do not emulate Arc's native USDC precompile or prove production safety.
...
Circle Bridge Kit resolves CCTP contracts, approvals, burn, attestation and mint. Bridging funds the user's Arc wallet; a checkout payment remains a separate explicit signature. The source wallet needs test ETH; Arc mint transactions need Arc USDC gas. Returned recovery results are stored in sessionStorage and can be downloaded. Keep the tab open during the initial bridge operation; closing before the SDK returns may require recovery using the source transaction and Circle's CCTP tooling. Cross-chain end-to-end execution requires source-chain test tokens and gas and has not been verified by the local contract suite.
